...
import {getNetwork, isAuthenticated} from "../utils/helpers";
import {
    NetworkCreate, NetworkCreateType,
    NetworkEdit, NetworkEditType,
    Network, NetworkType,
    NetworkUserPermission, NetworkUserPermissionType,
    NetworkTokens, NetworkTokensType,
    NetworkTokenName, NetworkTokenNameType,
    FacebookTokens, FacebookTokensType,
    MastodonTokens, MastodonTokensType,
    BlueskyTokens, BlueskyTokensType,
    TwitterTokens, TwitterTokensType,
    ThreadsTokens, ThreadsTokensType,
...
async function routes (fastify: FastifyInstance /*, options: any*/) {
     ...
     */
    // Přidání nové sítě
    fastify.post<{ Body: NetworkCreateType, Reply: NetworkType | IReply }>(
        '/network/add',
        {
     ...
     */
    // Smazání sítě
    fastify.delete<{ Params: { networkId: number }, Reply: IReply }>(
        ...
        async (request, reply) => {
            try {
                ...
                if (network.owner_id !== request.session.userId) return reply.status(403).send({ error: 'Forbidden' });
    ...
    fastify.put<{ Params: { networkId: number }, Body: NetworkEditType, Reply: NetworkType | IReply }>(
    ...
    fastify.get<{ Params: { networkId: number }, Reply: NetworkType | IReply }>(
        '/network/:networkId',
        {
            preValidation: isAuthenticated,
            schema: {
        ...
        async (request, reply) => {
            try {
                ...
                let permission: 'read' | 'write' | 'admin' | undefined;
                if (network.owner_id === request.session.userId) {
                    ...
                    permission = userAccess?.permission as 'read' | 'write' | undefined;
                }
    ...
    fastify.get<{ Reply: NetworkType[] | IReply }>(
        '/network/list/owned',
        {
            preValidation: isAuthenticated,
            schema: {
    ...
    fastify.get<{ Reply: NetworkType[] | IReply }>(
        ...
        async (request, reply) => {
            try {
                ...
                const accessibleNetworkMap = new Map(
                    accessibleNetworks.map(access => [access.networks_id, access.permission])
                );
    ...
    fastify.post<{ Params: { networkId: number }, Body: NetworkUserPermissionType, Reply: IReply }>(
        '/network/:networkId/addperm',
        {
            preValidation: isAuthenticated,
            schema: {
    ...
    fastify.delete<{ Params: { networkId: number, granteeId: number }, Reply: IReply }>(
        '/network/:networkId/removeperm/:granteeId',
        {
            preValidation: isAuthenticated,
            schema: {
    ...
    fastify.put<{ Params: { networkId: number }, Body: NetworkUserPermissionType, Reply: IReply }>(
        '/network/:networkId/changeperm',
        {
            preValidation: isAuthenticated,
            schema: {
    ...
    fastify.get<{ Params: { networkId: number }, Reply: NetworkUserPermissionType[] | IReply }>(
        '/network/:networkId/listperm',
        {
            preValidation: isAuthenticated,
            schema: {
                ...
                response: {
                    200: Type.Array(NetworkUserPermission)
                }
            },
        },
        async (request, reply) => {
            try {
                ...
                reply.status(200).send(
                    users.map(user => ({
                            ...
                            permission: user.permission as "read" | "write"
                        }
                    ))
                );
    ...
    fastify.post<{ Params: { networkId: number }, Body: NetworkTokensType | FacebookTokensType | MastodonTokensType | BlueskyTokensType | TwitterTokensType | ThreadsTokensType, Reply: IReply }>(
        '/network/:networkId/addtoken',
        {
            preValidation: isAuthenticated,
            schema: {
                ...
                body: Type.Union([NetworkTokens, FacebookTokens, MastodonTokens, BlueskyTokens, TwitterTokens, ThreadsTokens]),
                response: {
                    200: { success: Type.Boolean(), message: Type.String() }
                }
            },
        },
        async (request, reply) => {
            try {
                const { networkId } = request.params;
                const requestBody = request.body as NetworkTokensType | FacebookTokensType | MastodonTokensType | BlueskyTokensType | TwitterTokensType | ThreadsTokensType;
                ...
                if (network.network_type === 'facebook') {
                    ...
                    if ('appId' in requestBody && 'appSecret' in requestBody && 'pageId' in requestBody && 'shortLivedUserAccessToken' in requestBody) {
                        ...
                        const facebookManager = new FacebookTokenManager();
                        const result = await facebookManager.initializeFacebookTokens(networkId, requestBody as FacebookTokensType);
                    ...
                    if ('instanceUrl' in requestBody && 'accessToken' in requestBody) {
                        ...
                        const mastodonTokens = requestBody as MastodonTokensType;
                    ...
                    if ('handle' in requestBody && 'password' in requestBody) {
                        ...
                        const blueskyTokens = requestBody as BlueskyTokensType;
                        await prisma.networkToken.createMany({
                            data: [
                                {
                                    network_id: networkId,
                    ...
                    if ('apiKey' in requestBody && 'apiSecret' in requestBody && 'accessToken' in requestBody && 'accessTokenSecret' in requestBody) {
                        ...
                        const twitterTokens = requestBody as TwitterTokensType;
                        await prisma.networkToken.createMany({
                            data: [
                                {
                                    network_id: networkId,
                    ...
                    if ('threadsUserId' in requestBody && 'threadsAppSecret' in requestBody && 'longLivedAccessToken' in requestBody) {
                        ...
                        const threadsTokens = requestBody as ThreadsTokensType;
    ...
    fastify.get<{ Params: { networkId: number }, Reply: NetworkTokenNameType[] | IReply }>(
        '/network/:networkId/listtokens',
        {
            preValidation: isAuthenticated,
            schema: {
                ...
                response: {
                    200: Type.Array(NetworkTokenName)
                }
            },
        },
        async (request, reply) => {
    ...
    fastify.get<{
        ...
        Reply: PaginatedDetailedPostsType | IReply
    }>(
        ...
        async (request, reply) => {
            try {
                ...
                const postsMap = new Map<number, PostDetailType>();
    ...
    fastify.get<{
        Params: { networkId: number, postId: number },
        Reply: PostDetailType | IReply
    }>(
        '/network/:networkId/posts/:postId',
        {
            preValidation: isAuthenticated,
            schema: {
    ...
    fastify.get<{
        ...
        Reply: NetworkType[] | IReply
    }>(
        '/post/:postId/availablenetworks',
        {
            preValidation: isAuthenticated,
            schema: {
        ...
        async (request, reply) => {
            try {
                ...
                const commonNetworks = currentUserNetworks.filter(network =>
                    creatorNetworkIds.includes(network.id)
                );
                ...
                const availableNetworks = commonNetworks.map(network => ({
                    ...
                    permission: network.owner_id === currentUserId ? 'admin' as const : 'write' as const
                }));
