...
const SocialNetworkProviderFactory_1 = require("../social/SocialNetworkProviderFactory");
...
async function routes(fastify /*, options: any*/) {
    ...
    async function getPostDetail(postId, userId) {
        try {
            ...
            const hasBasicAccess = await (0, helpers_1.hasPostPermission)(postId, userId);
            ...
            if (!hasBasicAccess) {
                ...
                for (const networkId of uniqueNetworkIds) {
                    const hasReadAccess = await (0, helpers_1.hasNetworkReadAccess)(networkId, userId);
                    if (hasReadAccess) {
            ...
            for (const content of allContents) {
                let canView = await (0, helpers_1.canViewContent)(content.id, userId);
                ...
                if (!canView) {
                    ...
                    for (const link of linkedNetworks) {
                        const hasReadAccess = await (0, helpers_1.hasNetworkReadAccess)(link.networks_id, userId);
                        if (hasReadAccess) {
                            canView = true;
                            break;
                        }
                ...
                if (canView) {
                    ...
                    const canEdit = await (0, helpers_1.canEditContent)(content.id, userId);
            ...
            for (const attachment of allAttachments) {
                let canView = await (0, helpers_1.canViewAttachment)(attachment.id, userId);
                // Pokud uživatel nemůže zobrazit přes standardní oprávnění, kontroluje přístup ke čtení propojených sítí
                if (!canView) {
                ...
                if (canView) {
                    ...
                    const filePath = node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString(), node_path_1.default.basename(attachment.path));
                    try {
     ...
     */
    fastify.post('/post/:postId/newcontent', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            description: 'Vytvoření nového obsahu příspěvku',
            tags: ['post'],
            body: schemas_1.NewPostContent,
            params: typebox_1.Type.Object({
                postId: typebox_1.Type.Integer()
            }),
            response: {
                200: schemas_1.PostContent
            },
        },
    }, async (request, reply) => {
        try {
            ...
            const hasPermission = await (0, helpers_1.hasPostPermission)(postId, request.session.userId);
            if (!hasPermission)
            ...
            const canEdit = await (0, helpers_1.canEditContent)(newContent.id, request.session.userId);
            reply.status(200).send({
                id: newContent.id,
                postId: postId,
                content: content,
     ...
     */
    fastify.put('/post/:postId/editcontent', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            body: schemas_1.EditPostContent,
            response: {
                200: schemas_1.PostContent
            },
        },
        ...
        try {
            ...
            if (!existingContent)
                return reply.status(400).send({ error: 'Post content not found' });
            // Kontroluje oprávnění uživatele - tři typy uživatelů mohou editovat:
            // 1. Tvůrce/editor příspěvku
            const hasPostAccess = await (0, helpers_1.hasPostPermission)(postId, userId);
            ...
            if (hasPostAccess) {
                ...
                // Kontroluje, zda může uživatel editovat tento obsah
                const canEdit = await (0, helpers_1.canEditContent)(updatedContent.id, request.session.userId);
            ...
            if (isNetworkOwner) {
                ...
                const hasAccessToAll = allNetworkIds.every(networkId => userAccessibleNetworks.includes(networkId));
                if (hasAccessToAll) {
                    const updatedContent = await prisma.content.update({
                        where: {
                            id: contentId
     ...
     */
    fastify.get('/post/:postId/listcontents', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            response: {
                200: typebox_1.Type.Array(schemas_1.PostContent)
            },
        },
    }, async (request, reply) => {
        const { postId } = request.params;
        ...
        for (const content of contents) {
            const canView = await (0, helpers_1.canViewContent)(content.id, request.session.userId);
            if (canView) {
                // Kontroluje, zda může uživatel editovat tento obsah
                const canEdit = await (0, helpers_1.canEditContent)(content.id, request.session.userId);
     ...
     */
    fastify.delete('/post/:postId/deletecontent/:contentId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        try {
            ...
            if (!content)
                return reply.status(400).send({ error: 'Content not found' });
            // Kontroluje, zda je uživatel tvůrcem příspěvku - pouze oni mohou mazat obsah
            const isPostCreator = await (0, helpers_1.isPostOwner)(postId, userId);
            if (!isPostCreator) {
                return reply.status(403).send({
     ...
     */
    fastify.delete('/post/:postId/delete', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const isOwner = await (0, helpers_1.isPostOwner)(postId, userId);
        if (isOwner) {
            ...
            if (postedContent)
                return reply.status(400).send({ error: 'Post cannot be deleted as it is already posted' });
            // Smaže příspěvek
     ...
     */
    fastify.post('/post/:postId/uploadfile', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const fileExtension = node_path_1.default.extname(data.filename).toLowerCase();
        if (!allowedExtensions.includes(fileExtension)) {
            return reply.status(400).send({
                error: `Unsupported file type. Allowed types: ${allowedExtensions.join(', ')}`
            });
        }
        const fileName = node_path_1.default.basename(`${Date.now()}.${data.filename.split('.').pop()}`);
        const filePath = node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString(), fileName);
        try {
            await fs_1.promises.mkdir(node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString()), { recursive: true });
     ...
     */
    fastify.delete('/post/:postId/removefile/:attachmentId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const filePath = node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString(), node_path_1.default.basename(attachment.path));
        try {
            await fs_1.promises.access(filePath, fs_1.constants.R_OK | fs_1.constants.W_OK);
     ...
     */
    fastify.get('/post/:postId/listfiles', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            response: {
                200: typebox_1.Type.Array(schemas_1.Attachment)
            },
        },
    }, async (request, reply) => {
        const { postId } = request.params;
        ...
        for (const attachment of attachments) {
            const canView = await (0, helpers_1.canViewAttachment)(attachment.id, request.session.userId);
            if (canView) {
                // Kontroluje, zda soubor existuje na disku
                const filePath = node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString(), node_path_1.default.basename(attachment.path));
                try {
     ...
     */
    fastify.get('/post/:postId/getfile/:attachmentId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const canView = await (0, helpers_1.canViewAttachment)(attachmentId, request.session.userId);
        if (!canView)
            return reply.status(400).send({ error: 'Attachment not found or not authorized' });
        // Kontroluje, zda příloha existuje
        const attachment = await prisma.attachment.findFirst({
        ...
        const safeFileName = node_path_1.default.basename(attachment.path);
        const filePath = node_path_1.default.join(uploadsDir, String(postId), safeFileName);
        try {
            await fs_1.promises.access(filePath, fs_1.constants.R_OK);
            // Nastaví správné Content-Type hlavičky
            const fileExtension = node_path_1.default.extname(safeFileName).toLowerCase();
     ...
     */
    fastify.post('/post/:postId/addeditor/:userId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const hasPermission = await (0, helpers_1.isPostOwner)(postId, request.session.userId);
        if (!hasPermission)
            return reply.status(400).send({ error: 'Post not found or not authorized' });
     ...
     */
    fastify.delete('/post/:postId/removeeditor/:userId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
     ...
     */
    fastify.post('/post/:postId/linkcontent/:contentId/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        if (!postContent)
            return reply.status(400).send({ error: 'Post content not found or not authorized' });
        // Kontroluje, zda má uživatel write přístup k síti (aktualizovaná logika)
        const hasNetworkAccess = await (0, helpers_1.hasNetworkWriteAccess)(networkId, request.session.userId);
        if (!hasNetworkAccess)
            return reply.status(400).send({ error: 'User not authorized to post to the network' });
        ...
        if (existingLink)
            return reply.status(400).send({ error: 'Content is already linked to this network' });
     ...
     */
    fastify.delete('/post/:postId/unlinkcontent/:contentId/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const { canDelete, canUnlink } = await (0, helpers_1.canDeleteContent)(contentId, request.session.userId, networkId);
     ...
     */
    fastify.get('/post/:postId/linkedcontents', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            response: {
                200: typebox_1.Type.Array(schemas_1.LinkedContent)
            },
        },
    }, async (request, reply) => {
        const { postId } = request.params;
        ...
        for (const content of contents) {
            const canView = await (0, helpers_1.canViewContent)(content.contents_id, request.session.userId);
            if (canView) {
     ...
     */
    fastify.post('/post/:postId/linkattachment/:attachmentId/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        if (postedContent)
            return reply.status(400).send({ error: 'Attachments cannot be added to post as it is already posted' });
     ...
     */
    fastify.delete('/post/:postId/unlinkattachment/:attachmentId/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        if (postedContent)
            return reply.status(400).send({ error: 'Attachments cannot be removed from post as it is already posted' });
    ...
    fastify.get('/post/:postId/linkedattachments', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            response: {
                200: typebox_1.Type.Array(schemas_1.LinkedAttachment)
            },
        },
    }, async (request, reply) => {
        const { postId } = request.params;
        ...
        for (const attachment of attachments) {
            const canView = await (0, helpers_1.canViewAttachment)(attachment.attachments_id, request.session.userId);
            if (canView) {
     ...
     */
    fastify.post('/post/:postId/send/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        try {
            ...
            const hasWriteAccess = await (0, helpers_1.hasNetworkWriteAccess)(networkId, request.session.userId);
            if (!hasWriteAccess) {
            ...
            const provider = SocialNetworkProviderFactory_1.SocialNetworkProviderFactory.getProvider(linkedContent.networks.network_type);
            if (!provider) {
            ...
            for (const attachment of linkedAttachments) {
                const filePath = node_path_1.default.join(process.env.UPLOADS_DIR, postId.toString(), node_path_1.default.basename(attachment.attachments.path));
                fastify.log.info(`[API Send] Checking attachment file: ${filePath}`);
                try {
                    await fs_1.promises.access(filePath, fs_1.constants.R_OK);
                    attachmentPaths.push(filePath);
                    fastify.log.info(`[API Send] Attachment verified and added: ${node_path_1.default.basename(filePath)}`);
                }
                catch (err) {
            ...
            const networkPostId = await provider.sendPost(linkedContent.contents.content, attachmentPaths, tokens);
     ...
     */
    fastify.post('/post/:postId/send/all', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        try {
            ...
            for (let i = 0; i < linkedContents.length; i++) {
                ...
                try {
                    ...
                    const hasWriteAccess = await (0, helpers_1.hasNetworkWriteAccess)(linkedContent.networks_id, request.session.userId);
                    if (!hasWriteAccess) {
                    ...
                    for (const attachment of linkedAttachments) {
                        ...
                        try {
                            await fs_1.promises.access(filePath, fs_1.constants.R_OK);
                            attachmentPaths.push(filePath);
                            fastify.log.info(`[API Send All] Attachment verified: ${node_path_1.default.basename(filePath)}`);
                        }
                        catch (err) {
     ...
     */
    fastify.post('/post/:postId/schedule/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            body: schemas_1.ScheduleContentRequest,
            response: {
                200: { success: true, message: typebox_1.Type.String() },
            },
        },
        ...
        try {
            ...
            const canSchedule = await (0, helpers_1.canScheduleContent)(linkedContent.contents_id, networkId, request.session.userId);
            if (!canSchedule) {
     ...
     */
    fastify.delete('/post/:postId/unschedule/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
     ...
     */
    fastify.get('/post/:postId/scheduled/:networkId', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
        ...
        const canView = await (0, helpers_1.canViewContent)(linkedContent.contents_id, request.session.userId);
        if (!canView) {
    ...
    fastify.get('/post/:postId', {
        ...
        try {
            ...
            // Kontroluje základní přístup k příspěvku - uživatel musí být vlastník nebo editor
            const hasBasicAccess = await (0, helpers_1.hasPostPermission)(postId, request.session.userId);
            // Pokud uživatel není vlastník/editor, kontroluje, zda má přístup přes oprávnění ke čtení sítě
            let hasNetworkAccess = false;
            if (!hasBasicAccess) {
                // Získá všechny sítě propojené s tímto příspěvkem
                ...
                // Kontroluje, zda má uživatel přístup ke čtení kterékoliv z těchto sítí
                for (const networkId of uniqueNetworkIds) {
                    const hasReadAccess = await (0, helpers_1.hasNetworkReadAccess)(networkId, request.session.userId);
                    if (hasReadAccess) {
                        hasNetworkAccess = true;
                        break;
                    }
            ...
            const allowedContents = [];
            for (const content of allContents) {
                let canView = await (0, helpers_1.canViewContent)(content.id, request.session.userId);
                // Pokud uživatel nemůže zobrazit přes standardní oprávnění, kontroluje přístup ke čtení propojených sítí
                if (!canView) {
                    const linkedNetworks = await prisma.postedContent.findMany({
                        where: {
                    ...
                    // Kontroluje, zda má uživatel přístup ke čtení kterékoliv propojené sítě
                    for (const link of linkedNetworks) {
                        const hasReadAccess = await (0, helpers_1.hasNetworkReadAccess)(link.networks_id, request.session.userId);
                        if (hasReadAccess) {
                            canView = true;
                            break;
                        }
            ...
            const allowedAttachments = [];
            for (const attachment of allAttachments) {
                let canView = await (0, helpers_1.canViewAttachment)(attachment.id, request.session.userId);
                // Pokud uživatel nemůže zobrazit přes standardní oprávnění, kontroluje přístup ke čtení propojených sítí
                if (!canView) {
                    const linkedNetworks = await prisma.postedContentAttachment.findMany({
                        where: {
     ...
     */
    fastify.get('/post/list/detailed', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
     ...
     */
    fastify.get('/post/editorlist/detailed', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
     ...
     */
    fastify.get('/post/list/filtered', {
        preValidation: helpers_1.isAuthenticated,
        schema: {
            ...
            querystring: typebox_1.Type.Object({
                page: typebox_1.Type.Optional(typebox_1.Type.Integer({ minimum: 1, default: 1 })),
                limit: typebox_1.Type.Optional(typebox_1.Type.Integer({ minimum: 1, maximum: 200, default: 10 })),
                networkId: typebox_1.Type.Optional(typebox_1.Type.Integer()),
        ...
        try {
            ...
            for (const post of allPosts) {
                ...
                const hasNetworkAccess = await (0, helpers_1.hasNetworkReadAccess)(post.networks_id, userId);
                if (hasNetworkAccess) {
                    accessiblePostIds.add(postId);
                }
            }
     ...
     */
    fastify.get('/post/attachmentsize', {
        schema: {
            description: 'Získání maximální povolené velikosti souboru pro nahrávání',
            tags: ['post'],
            response: {
                200: typebox_1.Type.Object({
                    maxFileSize: typebox_1.Type.Integer({ description: 'Maximální velikost souboru v bajtech' })
                })
            },
        },
    }, async (request, reply) => {
