...
const mockUploadBlob = jest.fn() as jest.MockedFunction<any>;

// Použije doMock pro zajištění správné náhrady modulu
jest.doMock('@atproto/api', () => ({
  AtpAgent: function MockAtpAgent(config: any) {
    return {
      login: mockLogin,
      post: mockPost,
...
jest.mock('sharp', () => {
  const mockSharpInstance = {
    ...
    toBuffer: jest.fn() as jest.MockedFunction<any>,
    metadata: jest.fn() as jest.MockedFunction<any>,
  };
...
import { BlueskyProvider } from '../src/social/BlueskyProvider';

describe('BlueskyProvider', () => {
  let blueskyProvider: BlueskyProvider;
  ...
  beforeEach(() => {
    ...
    // Create new instance
    blueskyProvider = new BlueskyProvider();
  ...
  describe('Token Validation', () => {
    ...
    it('should reject null/undefined tokens', () => {
      expect(blueskyProvider.validateTokens(null as any)).toBe(false);
      expect(blueskyProvider.validateTokens(undefined as any)).toBe(false);
    });
  ...
  describe('sendPost Method', () => {
    ...
    it('should send text-only post successfully', async () => {
      ...
      try {
        const result = await blueskyProvider.sendPost(
          'Test post content',
          [],
          validTokens
        );
  ...
  describe('getPostPerformance Method', () => {
    ...
    it('should return null (monitoring not fully implemented)', async () => {
      const result = await blueskyProvider.getPostPerformance(
        'at://did:plc:123/app.bsky.feed.post/abc123',
        validTokens
      );
