...
const TwitterProvider_1 = require("../src/social/TwitterProvider");
const logger_1 = require("../src/utils/logger");
const mockedLogger = logger_1.logger;
// Helper funkce pro vytvoření správně typovaných mock odpovědí
const createMockResponse = (data, ok = true, status = ok ? 201 : 400, statusText = ok ? 'Created' : 'Bad Request') => {
...
describe('TwitterProvider', () => {
    ...
    beforeEach(() => {
        ...
        twitterProvider = new TwitterProvider_1.TwitterProvider();
    ...
    describe('Token Validation', () => {
        ...
        it('should reject null/undefined tokens', () => {
            expect(twitterProvider.validateTokens(null)).toBe(false);
            expect(twitterProvider.validateTokens(undefined)).toBe(false);
        });
    });
    describe('sendPost Method', () => {
        ...
        it('should send text-only post successfully', async () => {
            ...
            const result = await twitterProvider.sendPost('Test post content', [], validTokens);
            expect(result).toBe('twitter_post_123');
            expect(mockFetch).toHaveBeenCalledWith('https://api.twitter.com/2/tweets', expect.objectContaining({
                method: 'POST',
        ...
        it('should ignore attachments and warn user', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            const result = await twitterProvider.sendPost('Test with attachments', ['/path/to/image.jpg', '/path/to/video.mp4'], validTokens);
            expect(result).toBe('twitter_post_123');
            expect(mockedLogger.warn).toHaveBeenCalledWith('[TwitterProvider] Ignoring 2 attachments - media upload disabled');
        ...
        it('should handle invalid tokens gracefully', async () => {
            ...
            const result = await twitterProvider.sendPost('Test content', [], invalidTokens);
        ...
        it('should handle API error response with detail', async () => {
            ...
            const result = await twitterProvider.sendPost('Test content', [], validTokens);
            expect(result).toBeNull();
        });
        ...
        it('should handle empty content', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            const result = await twitterProvider.sendPost('', [], validTokens);
        ...
        it('should handle null content', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            const result = await twitterProvider.sendPost(null, [], validTokens);
        ...
        it('should configure OAuth correctly', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            await twitterProvider.sendPost('OAuth test', [], validTokens);
    ...
    describe('getPostPerformance Method', () => {
        ...
        it('should return empty metrics (monitoring disabled)', async () => {
            const result = await twitterProvider.getPostPerformance('twitter_post_123', validTokens);
        ...
        it('should handle invalid tokens', async () => {
            ...
            const result = await twitterProvider.getPostPerformance('twitter_post_123', invalidTokens);
            expect(result).toBeNull();
        });
        it('should handle empty post ID', async () => {
            const result = await twitterProvider.getPostPerformance('', validTokens);
            expect(result).toEqual({
                postId: '',
                networkType: 'twitter',
                timestamp: expect.any(Date),
        ...
        it('should handle null post ID', async () => {
            const result = await twitterProvider.getPostPerformance(null, validTokens);
            expect(result).toEqual({
                postId: null,
                networkType: 'twitter',
                timestamp: expect.any(Date),
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        it('should handle very long content (within Twitter limits)', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            const result = await twitterProvider.sendPost(longContent, [], validTokens);
        ...
        it('should handle special characters and emojis', async () => {
            ...
            mockFetch.mockResolvedValue(mockResponse);
            const result = await twitterProvider.sendPost(specialContent, [], validTokens);
        ...
        it('should handle JSON parsing error', async () => {
            ...
            const mockJsonError = globals_1.jest.fn();
