...
// Mock path
const mockPath = {
  basename: jest.fn((filePath: string) => filePath.split('/').pop() || 'file.jpg')
};
...
import { MastodonProvider } from '../src/social/MastodonProvider';
...
describe('MastodonProvider', () => {
  let mastodonProvider: MastodonProvider;

  beforeEach(() => {
    jest.clearAllMocks();
    mastodonProvider = new MastodonProvider();
  ...
  describe('Token Validation', () => {
    ...
    it('should reject null/undefined tokens', () => {
      expect(mastodonProvider.validateTokens(null as any)).toBe(false);
      expect(mastodonProvider.validateTokens(undefined as any)).toBe(false);
    });
  });
  ...
  describe('sendPost Method', () => {
    ...
    it('should send text-only post successfully', async () => {
      ...
      const result = await mastodonProvider.sendPost(
        'Test post content',
        [],
        validTokens
      );
  ...
  describe('getPostPerformance Method', () => {
    ...
    it('should return performance metrics successfully', async () => {
      ...
      const result = await mastodonProvider.getPostPerformance(
        'mastodon_123',
        validTokens
      );
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith(
        'https://mastodon.social/api/v1/statuses/mastodon_123',
        {
          headers: {
            'Authorization': 'Bearer test_access_token',
