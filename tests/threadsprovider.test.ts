...
import { ThreadsProvider } from '../src/social/ThreadsProvider';
...
describe('ThreadsProvider', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    threadsProvider = new ThreadsProvider();
  ...
  describe('Token Validation', () => {
    ...
    it('should reject null/undefined tokens', () => {
      expect(threadsProvider.validateTokens(null as any)).toBe(false);
      expect(threadsProvider.validateTokens(undefined as any)).toBe(false);
    });
  });
  ...
  describe('sendPost Method', () => {
    ...
    it('should send text-only post successfully', async () => {
      ...
      const result = await threadsProvider.sendPost(
        'Test post content',
        [],
        validTokens
      );
      ...
      expect(mockedAxios.post).toHaveBeenNthCalledWith(1,
        'https://graph.threads.net/v1.0/test_user_id/threads',
        {
          media_type: 'TEXT',
      ...
      expect(mockedAxios.post).toHaveBeenNthCalledWith(2,
        'https://graph.threads.net/v1.0/test_user_id/threads_publish',
        {
    ...
    it('should ignore attachments and warn user', async () => {
      ...
      expect(mockedLogger.warn).toHaveBeenCalledWith(
        '[ThreadsProvider] Ignoring 2 attachments - only text posts supported'
      );
  ...
  describe('getPostPerformance Method', () => {
    ...
    it('should return empty metrics (monitoring disabled)', async () => {
      const result = await threadsProvider.getPostPerformance(
        'threads_post_123',
        validTokens
      );
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle different user IDs correctly', async () => {
      ...
      expect(mockedAxios.post).toHaveBeenNthCalledWith(1,
        'https://graph.threads.net/v1.0/different_user_456/threads',
        expect.any(Object)
      );

      expect(mockedAxios.post).toHaveBeenNthCalledWith(2,
        'https://graph.threads.net/v1.0/different_user_456/threads_publish',
        expect.any(Object)
      );
    });
  });
