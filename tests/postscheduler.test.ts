...
const mockPrisma = {
  postedContent: {
    ...
    update: jest.fn() as jest.MockedFunction<(args: any) => Promise<any>>,
  },
};
...
describe('PostScheduler', () => {
  ...
  beforeEach(() => {
    ...
    postScheduler = new PostScheduler(mockPrisma as any, 1);
  ...
  describe('Constructor and Basic Operations', () => {
    it('should create instance with default check interval', () => {
      const scheduler = new PostScheduler(mockPrisma as any);
    ...
    it('should create instance with custom check interval', () => {
      const scheduler = new PostScheduler(mockPrisma as any, 5);
      const status = scheduler.getStatus();
  ...
  describe('Scheduled Posts Processing', () => {
    ...
    it('should process scheduled posts successfully', async () => {
      ...
      expect(mockProvider.sendPost).toHaveBeenCalledWith(
        'Test post content',
  ...
  describe('Multiple Posts Processing', () => {
    it('should process multiple posts for different networks', async () => {
      ...
      const mockTwitterProvider = {
        ...
        sendPost: jest.fn() as jest.MockedFunction<(content: string, attachments: string[], tokens: any) => Promise<string | null>>,
        networkType: 'twitter',
        getRequiredTokens: jest.fn(() => ['accessToken', 'accessTokenSecret']) as jest.MockedFunction<() => string[]>,
        getPostPerformance: jest.fn() as jest.MockedFunction<(postId: string, tokens: any) => Promise<any>>,
        getMonitoringInterval: jest.fn(() => 1) as jest.MockedFunction<() => number>,
      };
      ...
      expect(mockProvider.sendPost).toHaveBeenCalledWith(
        'Facebook post',
        [],
        { pageAccessToken: 'fb_token', pageId: 'fb_page_id' }
      );

      expect(mockTwitterProvider.sendPost).toHaveBeenCalledWith(
        'Twitter post',
        [],
        { accessToken: 'tw_token', accessTokenSecret: 'tw_secret' }
      );
